...
export const useInstructorListStore = create<InstructorListStore>(
  (set, get) => ({
    instructors: [],
    isLoading: false,
    error: null,
      ...
      if (filterByMode === "semester") {
        if (selectedSemesterId !== null) {
          filtered = filtered.filter(
            (item) => item.relations.semester_id === selectedSemesterId,
          );
        }
        if (selectedDepartmentId !== null) {
          filtered = filtered.filter(
            (item) => item.relations.department_id === selectedDepartmentId,
          );
        }
      } else {
        if (selectedDepartmentId !== null) {
          ...
          if (selectedDept) {
            filtered = filtered.filter(
              (item) => item.instructor.field === selectedDept.name,
            );
          } else {
            filtered = [];
          }
    ...
    setSelectedSemesterId: (id: number | null) =>
      set({ selectedSemesterId: id }),
    setFilterByMode: (mode: "department" | "semester") =>
      set({ filterByMode: mode }),
  }),
);
