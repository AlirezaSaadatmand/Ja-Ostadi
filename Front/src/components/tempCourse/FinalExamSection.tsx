...
const FinalExamSection: React.FC<FinalExamSectionProps> = ({
  ...
  const inputClass =
    "w-full rounded-xl border border-gray-300 px-4 py-2 focus:outline-none focus:ring-2 focus:ring-[#AB8A58] bg-white";
  ...
  return (
    <div className="bg-white rounded-2xl shadow p-6 space-y-4">
      <h2 className="font-semibold text-lg text-gray-800">امتحان نهایی</h2>
                  ...
                  <div className="grid grid-cols-5 gap-2 max-h-32 overflow-y-auto">
                    {persianYears.map((y) => (
                      <button
                        ...
                        onClick={() =>
                          handleTempDateSelect(y, tempDate.month, tempDate.day)
                        }
                      >
                        {y}
                      </button>
                  ...
                  <div className="grid grid-cols-7 gap-2 max-h-32 overflow-y-auto">
                    {daysInMonth.map((d) => (
                      <button
                        ...
                        onClick={() =>
                          handleTempDateSelect(tempDate.year, tempDate.month, d)
                        }
                      >
                        {d}
                      </button>
                ...
                <div className="flex justify-between items-center pt-2 border-t">
                  <div className="text-sm font-medium text-gray-700">
                    ...
                    <span className="text-[#AB8A58] font-bold">
                      {tempDate.year}/
                      {tempDate.month.toString().padStart(2, "0")}/
                      {tempDate.day.toString().padStart(2, "0")}
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-4 gap-2 max-h-32 overflow-y-auto">
                    {hours.map((h) => (
                      <button
                        ...
                        onClick={() =>
                          handleTempTimeSelect(h, tempTime.minutes)
                        }
                      >
      ...
      <div className="text-xs text-gray-500 mt-2 flex flex-col gap-1">
        <div>
          📝 فرمت تاریخ: <span className="font-mono">سال/ماه/روز</span> مثال:{" "}
          <span className="font-mono">۱۴۰۳/۱۰/۱۵</span>
        </div>
        <div>
          📝 فرمت زمان: <span className="font-mono">ساعت:دقیقه</span> مثال:{" "}
          <span className="font-mono">۱۴:۳۰</span>
        </div>
        <div className="text-[#AB8A58] mt-1">
          💡 روی آیکون تقویم یا ساعت کلیک کنید تا انتخابگر باز شود
        </div>
      </div>
    </div>
  );
